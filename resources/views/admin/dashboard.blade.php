@php
  ...
  $lulus = $pendaftar->filter(fn($u) => optional($u->seleksi)->status === 'Lulus')->count();
  $tidakLulus = $pendaftar->filter(fn($u) => optional($u->seleksi)->status === 'Tidak Lulus')->count();
  $belumDiproses = $pendaftar->filter(fn($u) => $u->seleksi === null)->count();
@endphp
...
<head>
  ...
  <style>
    ...
    /* Status Badge Styles - Konsisten */
    .badge-success { @apply inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700 border border-green-300; }
    .badge-danger { @apply inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700 border border-red-300; }
    .badge-warning { @apply inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 border border-yellow-300; }
    .badge-info { @apply inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700 border border-blue-300; }
    .badge-gray { @apply inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-700 border border-gray-300; }
    
    /* Card Styles - Konsisten */
    .card { @apply bg-white border border-gray-200 rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow; }
...
<body class="bg-gray-50 font-hubot overflow-hidden">
  ...
  <header class="fixed top-0 left-0 right-0 bg-white border-b border-gray-200 z-40 no-print">
    <div class="flex items-center justify-between px-4 md:px-6 h-16">
      ...
      <div class="flex items-center gap-4">
        ...
        <button onclick="showLogoutModal()" class="p-2 hover:bg-gray-100 rounded-lg text-gray-600">
  ...
  <div class="flex h-screen pt-16">
        ...
        <ul class="space-y-1">
          ...
          <li>
            <a href="#" onclick="showPage('home')"
              class="nav-link flex items-center px-4 py-2.5 rounded-lg hover:bg-gray-50 text-gray-700 transition-colors bg-blue-50 text-blue-700">
          ...
          <li>
            <button
              class="w-full flex items-center justify-between px-4 py-2.5 rounded-lg hover:bg-gray-50 text-gray-700 transition-colors"
              onclick="toggleSubMenu('dataMenu')">
              <div class="flex items-center">
                <i class="fas fa-database w-5 text-gray-500 mr-3"></i>
                <span class="font-medium">Data</span>
              </div>
              <i id="dataMenuIcon" class="fas fa-chevron-down text-xs text-gray-400 transition-transform"></i>
            </button>
            <ul id="dataMenu" class="ml-8 mt-1 space-y-1 hidden">
              <li>
                <a href="#" onclick="showPage('dataPendaftar')"
                  class="flex items-center justify-between px-4 py-2 rounded-lg hover:bg-gray-50 text-gray-600 text-sm transition-colors">
                  <span class="flex items-center">
                    <span class="w-1.5 h-1.5 bg-blue-500 rounded-full mr-3"></span>
                    Data Pendaftar
                  </span>
                  <span class="bg-blue-100 text-blue-600 text-xs px-2 py-0.5 rounded-full font-medium">{{ $totalPendaftar }}</span>
                </a>
              </li>
              <li>
                <a href="#" onclick="showPage('dataAdmin')"
                  class="flex items-center justify-between px-4 py-2 rounded-lg hover:bg-gray-50 text-gray-600 text-sm transition-colors">
                  <span class="flex items-center">
                    <span class="w-1.5 h-1.5 bg-indigo-500 rounded-full mr-3"></span>
                    Data Admin
                  </span>
                  <span class="bg-indigo-100 text-indigo-600 text-xs px-2 py-0.5 rounded-full font-medium">{{ $totalAdmins }}</span>
                </a>
              </li>
            </ul>
          </li>
          ...
          <li>
            <button
              class="w-full flex items-center justify-between px-4 py-2.5 rounded-lg hover:bg-gray-50 text-gray-700 transition-colors"
              onclick="toggleSubMenu('seleksiMenu')">
              <div class="flex items-center">
                <i class="fas fa-clipboard-check w-5 text-gray-500 mr-3"></i>
                <span class="font-medium">Seleksi</span>
              </div>
              <i id="seleksiMenuIcon" class="fas fa-chevron-down text-xs text-gray-400 transition-transform"></i>
            </button>
            <ul id="seleksiMenu" class="ml-8 mt-1 space-y-1 hidden">
              <li>
                <a href="#" onclick="showPage('seleksiPeserta')"
                  class="flex items-center px-4 py-2 rounded-lg hover:bg-gray-50 text-gray-600 text-sm transition-colors">
                  <span class="w-1.5 h-1.5 bg-green-500 rounded-full mr-3"></span>
                  Seleksi Peserta
                </a>
              </li>
              <li>
                <a href="#" onclick="showPage('periodeSeleksi')"
                  class="flex items-center px-4 py-2 rounded-lg hover:bg-gray-50 text-gray-600 text-sm transition-colors">
          ...
          <li>
            <a href="#" onclick="showPage('settings')"
              class="nav-link flex items-center px-4 py-2.5 rounded-lg hover:bg-gray-50 text-gray-700 transition-colors">
    ...
    <!-- Main Content -->
    <main class="flex-1 overflow-y-auto min-w-0" style="max-height: calc(100vh - 4rem);">
      ...
      <div class="p-6">
        <div id="homePage" class="page-content">
          <div class="mb-8">
            ...
            <p class="text-gray-600">Kelola sistem PPDB dengan mudah dan efisien</p>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <p class="text-xs text-gray-500 mt-2">Administrator terdaftar</p>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <h3 class="text-sm font-medium text-gray-600 mb-1">Total Pendaftar</h3>
              <p class="text-3xl font-bold text-gray-800">{{ $totalPendaftar }}</p>
              <p class="text-xs text-gray-500 mt-2">Calon siswa terdaftar</p>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <h3 class="text-sm font-medium text-gray-600 mb-1">Menunggu Verifikasi</h3>
              <p class="text-3xl font-bold text-gray-800">{{ $belumDiproses }}</p>
              <p class="text-xs text-gray-500 mt-2">Perlu ditindaklanjuti</p>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <h3 class="text-sm font-medium text-gray-600 mb-1">Diterima</h3>
              <p class="text-3xl font-bold text-gray-800">{{ $lulus }}</p>
              <p class="text-xs text-gray-500 mt-2">Calon siswa diterima</p>
            </div>
          </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-6 hover:shadow-lg transition-shadow">
                <h2 class="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mr-2 text-blue-500" fill="currentColor"
                    viewBox="0 0 20 20">
                    <path
                ...
                <div class="grid grid-cols-2 gap-3">
                  <button onclick="showPage('dataPendaftar')"
                    class="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 hover:from-blue-100 hover:to-indigo-100 rounded-xl transition-all duration-200 transform hover:scale-105 group">
                    <div class="flex flex-col items-center text-center">
                      <div
                        class="w-12 h-12 bg-blue-500 rounded-xl flex items-center justify-center mb-2 group-hover:bg-blue-600 transition-colors">
                  ...
                  <button onclick="showPage('seleksiPeserta')"
                    class="p-4 bg-gradient-to-br from-green-50 to-emerald-50 hover:from-green-100 hover:to-emerald-100 rounded-xl transition-all duration-200 transform hover:scale-105 group">
                    <div class="flex flex-col items-center text-center">
                      <div
                        class="w-12 h-12 bg-green-500 rounded-xl flex items-center justify-center mb-2 group-hover:bg-green-600 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-white" fill="currentColor"
                          viewBox="0 0 20 20">
                  ...
                  <button onclick="showPage('periodeSeleksi')"
                    class="p-4 bg-gradient-to-br from-purple-50 to-pink-50 hover:from-purple-100 hover:to-pink-100 rounded-xl transition-all duration-200 transform hover:scale-105 group">
                    <div class="flex flex-col items-center text-center">
                      <div
                        class="w-12 h-12 bg-purple-500 rounded-xl flex items-center justify-center mb-2 group-hover:bg-purple-600 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-white" fill="currentColor"
                          viewBox="0 0 20 20">
                          <path fill-rule="evenodd"
                  ...
                  <button onclick="showPage('dataAdmin')"
                    class="p-4 bg-gradient-to-br from-orange-50 to-red-50 hover:from-orange-100 hover:to-red-100 rounded-xl transition-all duration-200 transform hover:scale-105 group">
                    <div class="flex flex-col items-center text-center">
                      <div
                        class="w-12 h-12 bg-orange-500 rounded-xl flex items-center justify-center mb-2 group-hover:bg-orange-600 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-white" fill="currentColor"
                          viewBox="0 0 20 20">
              ...
              <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl shadow-md p-6 text-white">
                ...
                <div class="space-y-4">
                  <div
                    class="flex items-start gap-3 p-3 bg-white bg-opacity-10 rounded-lg backdrop-blur-sm hover:bg-opacity-20 transition-all">
                  ...
                  <div
                    class="flex items-start gap-3 p-3 bg-white bg-opacity-10 rounded-lg backdrop-blur-sm hover:bg-opacity-20 transition-all">
                    <div class="w-2 h-2 bg-white rounded-full mt-2 flex-shrink-0 animate-pulse"></div>
                    <p class="text-sm">Hubungi administrator sistem jika mengalami kendala teknis atau memerlukan
                      bantuan</p>
                  </div>
                </div>
              </div>
            ...
            <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
              ...
              <div class="space-y-3">
                @forelse($pendaftar->take(5) as $index => $p)
                  <div
                    class="flex items-center gap-4 p-3 hover:bg-gray-50 rounded-lg transition-colors border border-gray-100">
                    <div
                      class="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 shadow-md">
                      {{ strtoupper(substr($p->nama_pendaftar ?? 'U', 0, 1)) }}
                    </div>
                    <div class="flex-1 min-w-0">
                      <p class="text-sm font-semibold text-gray-800 truncate">
                        {{ $p->nama_pendaftar ?? 'Nama tidak tersedia' }}
                      </p>
                    ...
                    <div class="flex-shrink-0">
                      @if($p->status_seleksi == 'Diterima')
                        <span
                          class="px-3 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded-full">Diterima</span>
                      ...
                      @else
                        <span
                          class="px-3 py-1 bg-amber-100 text-amber-700 text-xs font-semibold rounded-full">Pending</span>
                      @endif
                    </div>
                  </div>
                @empty
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all animate-slide-in">
      <div class="p-6 md:p-8">
        <div class="flex justify-center mb-6">
          <div
            class="w-20 h-20 bg-gradient-to-br from-red-100 to-red-200 rounded-full flex items-center justify-center shadow-lg">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
          <button type="button" onclick="closeLogoutModal()"
            class="flex-1 px-6 py-3 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition-all duration-200 font-semibold hover:border-gray-400">
            Batal
          </button>
          <form action="{{ route('logout') }}" method="POST" class="flex-1">
            @csrf
            <button type="submit"
              class="w-full px-6 py-3 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-xl hover:from-red-600 hover:to-red-700 transition-all duration-200 font-semibold shadow-md hover:shadow-lg">
              Ya, Logout
            </button>
          </form>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      @if($errors->any())
        Swal.fire({
          ...
          html: '<ul class="text-left list-disc pl-4 text-sm">@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul>',
          confirmButtonColor: '#3085d6'
        });
      @endif
    });
