<div class="md:ml-12 md:mr-12 md:mb-10 animate-fade-in">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div
            class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1">
        ...
        <div
            class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-green-100 text-sm font-medium mb-1">Aktif Hari Ini</p>
                    <p class="text-3xl font-bold">{{ $admins->where('last_login', '>=', now()->startOfDay())->count() }}
                    </p>
                </div>
                <div
                    class="w-14 h-14 bg-white bg-opacity-20 rounded-xl flex items-center justify-center backdrop-blur-sm">
        ...
        <div
            class="bg-gradient-to-br from-purple-500 to-pink-600 rounded-xl p-5 text-white shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 text-sm font-medium mb-1">Admin Baru</p>
                    <p class="text-3xl font-bold">{{ $admins->where('created_at', '>=', now()->subDays(7))->count() }}
                    </p>
                </div>
                <div
                    class="w-14 h-14 bg-white bg-opacity-20 rounded-xl flex items-center justify-center backdrop-blur-sm">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100">
        <div class="p-6 border-b border-gray-100">
            <div class="relative">
                ...
                <input type="text" id="searchInput" placeholder="Cari berdasarkan username, nama panitia, atau email..."
                    ...
                    <tr class="border-b border-gray-200">
                        <th class="px-6 py-4 text-left font-bold text-gray-700 uppercase tracking-wider text-xs">
                            <div class="flex items-center gap-2">
                                <span
                                    class="w-8 h-8 bg-indigo-100 rounded-lg flex items-center justify-center text-indigo-600">
                                    #
                                </span>
                            </div>
                        </th>
                        ...
                        <tr class="hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 transition-all duration-200 admin-row"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-full flex items-center justify-center text-white font-bold shadow-md">
                        ...
                        <tr id="emptyRow">
                            <td colspan="6" class="text-center py-16">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-gray-400 text-sm">Klik tombol "Tambah Admin" untuk menambahkan
                ...
                <div class="bg-gradient-to-br from-white to-gray-50 border-2 border-gray-100 rounded-xl p-5 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 admin-card"
                    ...
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex items-center gap-3 flex-1">
                            <div
                                class="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg text-sm">
                                {{ $index + 1 }}
                            </div>
                        ...
                        <div class="flex-shrink-0">
                            <span
                                class="px-3 py-1 bg-gradient-to-r from-green-400 to-emerald-500 text-white rounded-full text-xs font-semibold shadow-sm">
                                Admin
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-2 pt-4 border-t-2 border-gray-100">
                        ...
                        <button onclick="openConfirmModal(this, '{{ route('admin.delete', $admin->id) }}', 'DELETE')"
            ...
            @empty
                <div id="emptyCard" class="text-center py-16 border-2 border-dashed border-gray-200 rounded-xl">
                    <div class="flex flex-col items-center">
                        ...
                        <p class="text-gray-500 font-semibold text-lg mb-1">Belum ada admin</p>
                        <p class="text-gray-400 text-sm px-4">Klik tombol "Tambah Admin" untuk menambahkan administrator
                            baru</p>
                    </div>
                </div>
            @endforelse
...
<div id="addAdminModal"
    class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4 backdrop-blur-sm">
    <div
        class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 sm:p-8 relative max-h-[90vh] overflow-y-auto animate-slide-in">
        <button onclick="closeAddAdminModal()"
            class="absolute top-4 right-4 w-10 h-10 flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-all text-2xl">
            &times;
        </button>

        <div class="mb-6">
            <div
                class="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center mb-4 shadow-lg">
        ...
        <form method="POST" action="{{ route('admin.store') }}" class="space-y-5">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">
                    <i class="fa fa-lock text-indigo-500 mr-2"></i>Konfirmasi Password
                    <span class="text-red-500">*</span>
                </label>
                <input type="password" name="password_confirmation" required
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" placeholder="Ulangi password">
            </div>
...
<div id="editAdminModal"
    ...
    <div
        ...
        <div class="mb-6">
            <div
                class="w-16 h-16 bg-gradient-to-br from-amber-400 to-orange-500 rounded-2xl flex items-center justify-center mb-4 shadow-lg">
        ...
        <form id="editAdminForm" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nama_panitia" id="editAdminNamaPanitia"
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all"
                    placeholder="Nama lengkap panitia" required>
            </div>
